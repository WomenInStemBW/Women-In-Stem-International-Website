...
// Upload podcast episode image
export const uploadEpisodeImage = async (file) => {
  try {
    const fileExt = file.name.split('.').pop();
    const fileName = `${Math.random().toString(36).substring(2)}-${Date.now()}.${fileExt}`;
    const filePath = `podcast-episode-images/${fileName}`;
    ...
    console.error('Error uploading episode image:', error);
    return { data: null, error };
  }
};